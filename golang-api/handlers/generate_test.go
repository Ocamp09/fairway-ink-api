...
type MockGenerateService struct {
	GenerateStlFn func(ssid string, stlKey string, file io.Reader, filename string, scale string, logger zap.SugaredLogger) (string, error)
}

func (m *MockGenerateService) GenerateStl(ssid string, stlKey string, file io.Reader, filename string, scale string, logger *zap.SugaredLogger) (string, error) {
	if m.GenerateStlFn != nil {
		return m.GenerateStlFn(ssid, stlKey, file, filename, scale, *logger)
	}
	return "", nil
}
...
func TestGenerateStl(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			mockService: func() *MockGenerateService {
				return &MockGenerateService{
					GenerateStlFn: func(ssid, stlKey string, file io.Reader, filename, scale string, logger zap.SugaredLogger) (string, error) {
